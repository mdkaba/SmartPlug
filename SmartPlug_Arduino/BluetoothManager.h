  ...
  class MyCharacteristicCallbacks : public BLECharacteristicCallbacks{
    ...
    void onWrite(BLECharacteristic *pCharacteristic){
      ...
      if (EOTPos >= len || EOTPos < 0)
      {
        ...
        //IP deprecated when we switched to databas-only comms
        //Serial.println(owner->IP);
        //Attempt to connect here
        Serial.println("Connecting to wifi" );
        //Doesn't work, but I think my ESP32 is a dud lmao
        Connect(owner->SSID, owner->PW);

        pCharacteristic -> setValue("OK");
        owner->networkConnected = true;
        Serial.println("Set Value OK Passed");
      }
    }
